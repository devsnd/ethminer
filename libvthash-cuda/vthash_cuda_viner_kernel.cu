...
#include "vthash_cuda_viner_kernel_globals.h"
...
)
{
	switch (parallelHash)
	{
		case 1: vthash_search <1> <<<blocks, threads, sharedbytes, stream >>>(g_output, start_nonce); break;
		case 2: vthash_search <2> <<<blocks, threads, sharedbytes, stream >>>(g_output, start_nonce); break;
		case 4: vthash_search <4> <<<blocks, threads, sharedbytes, stream >>>(g_output, start_nonce); break;
		case 8: vthash_search <8> <<<blocks, threads, sharedbytes, stream >>>(g_output, start_nonce); break;
		default: vthash_search <4> <<<blocks, threads, sharedbytes, stream >>>(g_output, start_nonce); break;
	}
