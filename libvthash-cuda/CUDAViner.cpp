...
bool CUDAViner::init(const h256& seed)
{
	// take local copy of work since it may end up being overwritten by kickOff/pause.
	try {
		unsigned device = s_devices[index] > -1 ? s_devices[index] : index;

		cnote << "Initialising viner...";

		VthashAux::LightType light;
		light = VthashAux::light(seed);
...
void CUDAViner::setDevices(const unsigned* _devices, unsigned _selectedDeviceCount)
{
...
HwMonitor CUDAViner::hwmon()
{
	dev::vth::HwMonitor hw;
	if (nvmlh) {
	...
	unsigned dagCreateDevice)
{
	try
	{
		...
		set_constants(dag, dagSize128, light, lightSize64); //in vthash_cuda_viner_kernel.cu
	...
	const dev::vth::WorkPackage& w)
{
	...
	while (true)
	{
		...
		run_vthash_search(s_gridSize, s_blockSize, m_sharedBytes, stream, buffer, m_current_nonce, m_parallelHash);
		if (m_current_index >= s_numStreams)
		{
			if (found_count)
			{
				...
				{
					Result r = VthashAux::eval(w.seed, w.header, nonces[i]);
