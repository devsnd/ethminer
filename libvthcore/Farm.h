...
public:
	...
	bool start(std::string const& _sealer, bool mixed)
	{
		...
		if (!p_vashrateTimer) {
			p_vashrateTimer = new boost::asio::deadline_timer(m_io_service, boost::posix_time::milliseconds(1000));
			p_vashrateTimer->async_wait(boost::bind(&Farm::processHashRate, this, boost::asio::placeholders::error));
	...
	void stop()
	{
		...
		if (p_vashrateTimer) {
			p_vashrateTimer->cancel();
			p_vashrateTimer = nullptr;
		}
	}
	...
	void processHashRate(const boost::system::error_code& ec) {
		...
		// Restart timer 	
		p_vashrateTimer->expires_at(p_vashrateTimer->expires_at() + boost::posix_time::milliseconds(1000));
		p_vashrateTimer->async_wait(boost::bind(&Farm::processHashRate, this, boost::asio::placeholders::error));
	}
	...
	WorkingProgress const& miningProgress(bool hwmon = false) const
	{
		...
		p.hashes = 0;
		{
			...
			for (auto const& i : m_viners) {
				...
				if (hwmon)
					p.vinerMonitors.push_back(i->hwmon());
			}
		}
		...
		for (auto const& cp : m_lastProgresses) {
			p.ms += cp.ms;
			p.hashes += cp.hashes;
			for (unsigned int i = 0; i < cp.vinersHashes.size(); i++)
			{
...
private:
	...
	int m_vashrateSmoothInterval = 10000;
	std::thread m_serviceThread;  ///< The IO service thread.
	boost::asio::io_service m_io_service;
	boost::asio::deadline_timer * p_vashrateTimer = nullptr;
