...
h256 VthashAux::seedHash(unsigned _number)
{
...
uint64_t VthashAux::number(h256 const& _seedHash)
{
	VthashAux& ethash = VthashAux::get();
	Guard l(ethash.x_epochs);
	unsigned epoch = 0;
...
VthashAux::LightType VthashAux::light(h256 const& _seedHash)
{
...
VthashAux::LightAllocation::LightAllocation(h256 const& _seedHash)
{
	uint64_t blockNumber = VthashAux::number(_seedHash);
	light = vthash_light_new(blockNumber);
	if (!light)
		BOOST_THROW_EXCEPTION(ExternalFunctionFailure("vthash_light_new()"));
...
Result VthashAux::eval(h256 const& _seedHash, h256 const& _headerHash, uint64_t _nonce) noexcept
{
	try
	{
