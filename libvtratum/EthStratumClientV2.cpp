...
{
	cnote << "Connecting to vtratum server " << p_active->host + ":" + p_active->port;
	...
	if (error)
	{
		cerr << "Could not connect to vtratum server " << p_active->host + ":" + p_active->port + ", " << error.message();
		reconnect();
	}
	else
	{
		...
		switch (m_protocol) {
			...
			case STRATUM_PROTOCOL_ETHEREUMSTRATUM:
				os << "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": [\"vthviner/" << ETH_PROJECT_VERSION << "\",\"EthereumStratum/1.0.0\"]}\n";
				break;
		}
...
bool EthStratumClientV2::submitHashrate(string const & rate) {
	...
	string json = "{\"id\": 6, \"jsonrpc\":\"2.0\", \"method\": \"eth_submitHashrate\", \"params\": [\"" + rate + "\",\"0x" + this->m_submit_vashrate_id + "\"]}\n";
	std::ostream os(&m_requestBuffer);
	os << json;
	write(m_socket, m_requestBuffer);
	return true;
...
bool EthStratumClientV2::submit(Solution solution) {
	...
	if (m_protocol == STRATUM_PROTOCOL_ETHEREUMSTRATUM) {
		vinernonce = nonceHex.substr(m_extraNonceHexSize, 16 - m_extraNonceHexSize);
	}
	...
	case STRATUM_PROTOCOL_ETHPROXY:
		...
		break;
	case STRATUM_PROTOCOL_ETHEREUMSTRATUM:
		json = "{\"id\": 4, \"method\": \"mining.submit\", \"params\": [\"" + p_active->user + "\",\"" + solution.job.hex() + "\",\"" + vinernonce + "\"]}\n";
		break;
	}
	std::ostream os(&m_requestBuffer);
	os << json;
